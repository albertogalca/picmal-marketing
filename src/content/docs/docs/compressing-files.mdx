...
Convert and Compress both have a **Quality** column. Each row shows the preset that will run on that file: the one from Settings, until you pick another for that row alone. Compress one video hard and leave another balanced in the same batch, without running two passes.

It sets the bitrate and codec recipe, never the output format. The column was called **Preset** until 1.8.9, which led people to expect it to pick a second file type.
...
To change several rows at once, tick their checkboxes (Compress has the same checkboxes as Convert, plus a **Select all videos** shortcut) and set the preset from the selection bar. Those ticks also decide which files the next run covers; see [run only the files you tick](/docs/managing-the-queue#run-only-the-files-you-tick).
