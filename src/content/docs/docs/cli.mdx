...
`convert`, `compress`, `combine`, `split-pdf`, `images-to-pdf`, `merge-audio`, and `watch` need a valid Picmal license. The read-only commands (`presets`, `formats`, `license-status`, `version`) work without one.
