...
`convert`, `compress`, `combine`, `combine-videos`, `split-pdf`, `images-to-pdf`, `merge-audio`, `split-chapters`, and `watch` need a valid Picmal license. The read-only commands (`presets`, `formats`, `license-status`, `version`) work without one.
