...
Set the DPI (dots per inch) metadata on output images. This controls how large the image appears when printed or placed in a design tool — it does not change the pixel dimensions.

Enable **Set DPI** in the output settings and pick a preset or enter a custom value.
...
DPI is applied to both converted and compressed images. Audio and video files are not affected.
