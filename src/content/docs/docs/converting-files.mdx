...
You can convert a file to the same format it's already in. Every format lists itself as a target, though it's never the default pick. This is useful for resampling audio (for example, FLAC to FLAC at a different sample rate) without changing the codec, and for re-encoding an image at your compression quality — a WebP to WebP pass shrinks the file instead of pushing it out to PNG or JPG. Video works the same way: MP4 to MP4 lets you change codec, resolution, or frame rate without leaving the container you started in. Because a same-format pass with compression off can only leave the file the same size or bigger, Picmal always compresses these. Same-format conversions follow your convert naming pattern (default `{original}_converted`), so the output lands beside the original instead of overwriting it. Adjust the pattern, prefix, or suffix in **Settings → Output**; see [output and naming](/docs/output-and-naming).

## Apple icon files

Drop an `.icns` in and convert it to PNG, JPG, or any other image format. An icon file holds every size from 16 px up to 1024, and by default you get the largest one, named and placed like any other conversion.

To get a different size, open **Settings → Images → Icon output size**:

| Choice | What you get |
| ------ | ------------ |
| **Largest available** | One image at the biggest size inside the icon. The default. |
| **All sizes** | One file per embedded size, named `<name>-1024x1024.png`, `<name>-512x512.png`, and so on. |
| **1024 px** … **16 px** | One image at that size, if the icon holds it. |

Sizes stored twice inside the icon (a `256x256` and a `128x128@2x`, say) come out once. All the outputs land in the output folder you picked, not next to the source.

This works from the command line too, with the same choice on `--icon-size`:

```bash
picmal-cli convert -i icon.icns -f png -o ~/out
picmal-cli convert -i icon.icns -f png -o ~/out --icon-size 256
picmal-cli convert -i icon.icns -f png -o ~/out --icon-size all
```
