...
Drop any number of images into the folder and, ten seconds after the last one lands, Picmal writes a single PDF with one page per image, in Finder name order. Pick **Fit to Image**, **A4**, or **US Letter** pages and the JPEG quality for the embedded images in the folder's settings, name the PDF with the usual [naming tokens](/docs/output-and-naming#tokens), and turn on **Delete originals** if you want the images cleared once the PDF exists.
...
From the terminal, `picmal-cli watch add --action images-to-pdf` and `--action combine-pdf` set these up. See [Command line](/docs/cli#watch-add).
