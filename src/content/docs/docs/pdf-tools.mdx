...
Shrinking a PDF isn't one of these four tools — drop the file into **Compress** instead. Picmal re-encodes only the images inside the document, so text stays selectable, links keep working and vector graphics stay sharp. See [How PDF compression works](/docs/image-compression#how-pdf-compression-works).
