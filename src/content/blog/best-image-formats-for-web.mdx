...
In practice, WebP can replace both JPEG and PNG for most web use cases. It handles photographs, graphics with transparency, and even animated sequences (a better GIF, basically). With 97%+ browser support in 2025, the compatibility excuse is gone.
...
The downside is speed. AVIF encoding is slow. Converting a large batch of photos to AVIF will take noticeably longer than the same batch to WebP. Browser support sits around 90% in 2025, with Chrome, Firefox, and Safari all on board (Safari was the last holdout). It's no longer experimental, but it's not as bulletproof as WebP yet.
...
Picmal can batch convert your source files into AVIF (and generate the fallback formats at the same time). For a deeper look at how AVIF stacks up against everything else, see our guide to [the best image formats in 2025](https://picmal.app/blog/best-image-formats-2025).
...
Once you know which formats you need, the tedious part is actually converting the files. Doing it one at a time in Preview or an online tool wastes hours you could spend on actual work. Picmal converts entire folders at once, handles all the formats above, and runs locally on your Mac so nothing gets uploaded anywhere.