...
PDFs are compressed with the same text-preserving engine as the app: only the images inside are re-encoded, and the output is never larger than the input. See [How PDF compression works](/docs/image-compression#how-pdf-compression-works).
