...
To reduce picture size on a Mac in bulk, the `sips` loop in step 1 takes a whole folder of photos in one pass. If the goal is the Photos library as a whole rather than a few files, that is [how to reduce photo library size on Mac](/blog/reduce-photo-library-size-mac).
