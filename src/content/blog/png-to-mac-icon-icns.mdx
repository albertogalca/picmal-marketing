...
[Picmal](https://picmal.app) does all three from one source image in a single pass, macOS `.icns`, Windows `.ico`, and an Xcode-ready iOS `.appiconset`, with every size rendered and packaged. It runs on your Mac with no upload and it is $39 once. If you ship one icon a year, use the loop above.
