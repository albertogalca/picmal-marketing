...
description: Run Picmal from the terminal — convert, compress, combine PDFs, make PDFs from images, watch folders, and apply saved presets without opening the app.
---
...
`convert`, `compress`, `combine`, `images-to-pdf`, and `watch` need a valid Picmal license. The read-only commands (`presets`, `formats`, `license-status`, `version`) work without one.
