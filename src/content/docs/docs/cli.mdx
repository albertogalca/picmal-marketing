...
An `.icns` input fans out to one output per embedded size, named `<name>-<W>x<H>.<ext>` — the same convention `split-pdf` uses for pages. `--quality`, `--resize`, and `--watermark` apply to each size. See [Apple icon files](/docs/converting-files#apple-icon-files).
...
An image with no clear subject is skipped and reported, and the rest of the batch still runs. See [Remove Background](/docs/remove-background).
