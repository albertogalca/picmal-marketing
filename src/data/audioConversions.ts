...
function buildMetaDescription(
  ...
  if (to.name === "ALAC") {
    return `Convert ${from.name} to ALAC on Mac — lossless .m4a for Apple Music. Bit-perfect, batch, fully offline. One-time $39, no subscription.`;
  }
  if (to.name === "M4R") {
    return `Convert ${from.name} to M4R on Mac — make a custom iPhone ringtone. Fast, offline, batch. One-time $39, no subscription.`;
  }
  return `Convert ${from.name} to ${to.name} on Mac — bitrate and quality control, batch processing, fully offline. No upload limits. One-time $39, no subscription.`;
}
