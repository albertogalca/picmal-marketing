...
function buildBenefits(from: VideoFormatInfo, to: VideoFormatInfo): string[] {
  const common = [
    ...
    "One-time $15.99 — no monthly subscription",
  ];
  if (to.extension === "gif") {
    return [
...
function buildMetaDescription(
  ...
  if (to.extension === "gif") {
    return `Convert ${from.name} to GIF on Mac — turn video clips into looping animations with frame-rate and size control. Batch, offline, no watermark. One-time $15.99.`;
  }
  if (to.extension === "mp3") {
    return `Convert ${from.name} to MP3 on Mac — extract audio from video with bitrate control. Batch process offline, no upload limits. One-time $15.99, no subscription.`;
  }
  return `Convert ${from.name} to ${to.name} on Mac — H.264/H.265 with quality control, batch processing, fully offline. No watermark, no file-size limit. One-time $15.99.`;
}
