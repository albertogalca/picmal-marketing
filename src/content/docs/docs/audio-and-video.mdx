...
Only some formats can store a picture: **MP3, M4A, M4B, ALAC and FLAC**. WAV, AIFF, OGG and Opus have nowhere to put one, so converting to those leaves the output with no art — Picmal flags the row when that happens. Compressing a track keeps whatever art it already has, unless you also have **Remove metadata** turned on, which strips it along with everything else.
